...
% Updated: Jan 2020 by Tor; code maintenance
...
% imgs may be single 4-D file
if size(imgs, 1) == size(SETUP.X, 1) % 3D
    print_matrix(SETUP.X, [], imgs);
else % maybe 4D
    print_matrix(SETUP.X);
end
...
if exist(fullfile(pwd, 'weights.nii'), 'file')      % newer
    dat = fmri_data('weights.nii');
elseif exist(fullfile(pwd, 'weights.img'), 'file')  % legacy
    dat = fmri_data('weights.img');
else
    error('Cannot find weights.nii');
end
